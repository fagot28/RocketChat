package chat.rocket.luk.authentication.server.ui
...
import chat.rocket.luk.authentication.server.presentation.ServerPresenter
...
import chat.rocket.luk.util.extensions.hintContent
import chat.rocket.luk.util.extensions.inflate
import chat.rocket.luk.util.extensions.isValidUrl
import chat.rocket.luk.util.extensions.sanitize
import chat.rocket.luk.util.extensions.setLightStatusBar
...
class ServerFragment : Fragment(), ServerView {
    ...
    private fun setupSpinnerServer() {
        context?.let {
            spinner_server_name.adapter = ArrayAdapter<String>(
                    it,
                    android.R.layout.simple_dropdown_item_1line,
                        arrayOf("chat.technocom.tech",
                                "chat.technocom.tech",
                                "chat.technocom.tech"
                    )
            )

            text_server_url.textContent = "chat.technocom.tech"

            spinner_server_name.onItemSelectedListener =
                    object : AdapterView.OnItemSelectedListener {
                        override fun onItemSelected(
                                parent: AdapterView<*>?, view: View?, position: Int,
                                id: Long
                        ) {
                            text_server_url.textContent = parent?.getItemAtPosition(position).toString()
                        }
    ...
    private fun setupOnClickListener() =
        ui {
            button_connect.setOnClickListener {
                ...
                //presenter.checkServer(text_server_url.textContent)
            }
        }
